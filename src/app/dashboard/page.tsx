  ...
  return (
    <div className="min-h-screen bg-[#fafbfc]">
      ...
      <main className="pt-[100px] pb-12 w-[92%] max-w-[1240px] mx-auto">
        ...
        {/* Admin Cards: Pending Appts, Pending Certs, QR Code, Quick Links */}
        {isAdminOrNurse && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
              <div className="flex justify-between items-center px-4 pt-4 pb-2">
                <div className="flex items-center gap-2">
                  <h2 className="text-[13px] font-semibold text-[#111] m-0">Pending Appts</h2>
                ...
                <Link href="/admin/appointments" className="text-[10px] text-gray-400 no-underline font-medium hover:text-[#111] transition">View all</Link>
              </div>
              {(!pendingAppts.data || pendingAppts.data.length === 0) ? (
                <div className="px-4 pb-4 pt-1">
                  <div className="bg-gray-50/80 rounded-lg py-4 flex flex-col items-center">
                    <CalendarIcon size={16} className="text-gray-300 mb-1.5" />
                    <p className="text-gray-400 text-[11px] m-0">All clear</p>
                  </div>
                </div>
              ) : (
                <div className="px-4 pb-4 space-y-1.5">
                  {pendingAppts.data.slice(0, 3).map((appt: any) => (
                    <div key={appt.id} className="flex items-center justify-between py-1.5 border-b border-gray-50 last:border-0">
                      <div className="min-w-0 flex-1">
                        <p className="text-[11px] text-[#111] font-medium m-0 truncate">{appt.firstname} {appt.lastname}</p>
                        <p className="text-[10px] text-gray-400 m-0 truncate">{appt.purpose}</p>
                      </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
              <div className="flex justify-between items-center px-4 pt-4 pb-2">
                <div className="flex items-center gap-2">
                  <h2 className="text-[13px] font-semibold text-[#111] m-0">Pending Certs</h2>
                ...
                <Link href="/admin/certificates" className="text-[10px] text-gray-400 no-underline font-medium hover:text-[#111] transition">View all</Link>
              </div>
              {(!pendingCerts.data || pendingCerts.data.length === 0) ? (
                <div className="px-4 pb-4 pt-1">
                  <div className="bg-gray-50/80 rounded-lg py-4 flex flex-col items-center">
                    <CertificateIcon size={16} className="text-gray-300 mb-1.5" />
                    <p className="text-gray-400 text-[11px] m-0">All clear</p>
                  </div>
                </div>
              ) : (
                <div className="px-4 pb-4 space-y-1.5">
                  {pendingCerts.data.slice(0, 3).map((cert: any) => (
                    <div key={cert.id} className="flex items-center justify-between py-1.5 border-b border-gray-50 last:border-0">
                      <div className="min-w-0 flex-1">
                        <p className="text-[11px] text-[#111] font-medium m-0 truncate">{cert.firstname} {cert.lastname}</p>
                        <p className="text-[10px] text-gray-400 m-0 truncate">{cert.purpose}</p>
                      </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-4">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-6 h-6 rounded-[6px] bg-primary/10 flex items-center justify-center text-primary flex-shrink-0">
                  <QrCodeIcon size={12} />
                </div>
                <h2 className="text-[13px] font-semibold text-[#111] m-0">Registration QR</h2>
              </div>
              <div className="flex flex-col items-center">
                <div className="bg-white p-1.5 rounded-lg border border-gray-100 mb-2">
                  <img src={qrImageUrl} alt="Signup QR" width={100} height={100} className="block" />
                </div>
                <p className="text-[10px] text-gray-400 text-center mb-1.5 m-0">Scan to sign up</p>
                <a href={qrImageUrl} download="ISPSC-Signup-QR.png" target="_blank" rel="noopener noreferrer"
                  className="w-full flex items-center justify-center gap-1 bg-primary text-white border-none rounded-lg px-2 py-1.5 text-[11px] font-medium no-underline hover:bg-primary-hover transition">
            ...
            {/* Quick Links */}
            <div className="bg-white rounded-xl border border-gray-100 p-4">
              <h2 className="text-[13px] font-semibold text-[#111] mb-2 m-0">Quick Links</h2>
              <div className="space-y-0.5">
                <Link href="/admin/appointments" className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-50 no-underline text-gray-600 transition">
                  <CalendarIcon size={13} className="text-emerald-500" />
                  <span className="text-[12px]">Appointments</span>
                </Link>
                <Link href="/admin/certificates" className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-50 no-underline text-gray-600 transition">
                  <CertificateIcon size={13} className="text-purple-500" />
                  <span className="text-[12px]">Certificates</span>
                </Link>
                <Link href="/admin/users" className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-50 no-underline text-gray-600 transition">
        ...
        {/* Clinic Hours (Students only) */}
        {!isAdminOrNurse && accommodation.data && (
