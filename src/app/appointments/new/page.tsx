...
export default function NewAppointmentPage() {
  ...
  const [formData, setFormData] = useState({ appointment_date: "", purpose: "", remarks: "" });
      ...
      {loading && (
        <div className="fixed inset-0 bg-black/30 z-[9999] flex items-center justify-center">
          <div className="bg-white rounded-xl px-8 py-6 flex flex-col items-center gap-3">
            ...
            <p className="text-[14px] font-medium text-gray-600 m-0">Submitting appointment...</p>
          </div>
      ...
      <main className="pt-[100px] pb-10 flex justify-center">
        <div className="w-[900px] max-w-full bg-white border-[3px] border-primary rounded-[20px] overflow-hidden shadow-lg">
          ...
          <div className="bg-primary text-white text-center py-6 px-5">
            <h2 className="text-[30px] font-bold m-0">Request Appointment</h2>
            <p className="mt-2 text-[14px] opacity-90">Fill in the details below to schedule a clinic visit</p>
          </div>
          ...
          <div className="p-[35px]">
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-grid grid grid-cols-1 gap-5">
                <div className="flex flex-col">
                  <label className="mb-2 font-semibold text-[#333] text-[15px]">Appointment Date *</label>
                  <input type="date" value={formData.appointment_date} onChange={(e) => setFormData({ ...formData, appointment_date: e.target.value })} required min={today}
